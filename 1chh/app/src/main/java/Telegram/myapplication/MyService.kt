    ...
    private val newRequestFun = object : Runnable {
        override fun run() = Thread {
            ...
            (if (this@MyService.mess.isEmpty()) {
                ...
                URL(HOST + "1ch?limit=100&lastKnownId=${mess.last().num}")
            ...
            try {
                ...
                val tmp = JsonMapper
                    ...
                    .readValue<MutableList<MessageDTO>>(str)
                for (it in tmp) {
                    if (it.message.image?.link != null) {
                        try {
                            it.message.image.image =
                                decodeStream(URL(HOST + "img/${it.message.image.link}").openStream()).also { image ->
                    ...
                    putExtra(
                        ...
                        JsonMapper.builder().serializationInclusion(JsonInclude.Include.NON_NULL)
                            ...
                            .registerModule(KotlinModule.Builder().build()).writeValueAsString(
                                MessageDTO(
                                    user = "bach_bach",
                                    message = Data(null, Text(text = it.text.toString()))
                                )
                            )
                    )
...
//                    println("ABOBA")
//                    println(con.responseMessage + " " + con.responseCode)
                    con.disconnect()
                } catch (e: Exception) {
                    failedDB.insert(
